...
class StatsPerDagVdWeekActivity : Activity() {
    ...
    private fun initSwipes() {
        val sl = object : OnSwipeTouchListener(this@StatsPerDagVdWeekActivity) {
        ...
        val rlPerDagVdWeek = findViewById<RelativeLayout>(R.id.rlPerDagVdWeek)
        rlPerDagVdWeek.setOnTouchListener(sl)

        val bcPerDagVdWeek = findViewById<BarChart>(R.id.bcPerDagVdWeek)
        bcPerDagVdWeek.setOnTouchListener(sl)

        Helper.showMessage(this@StatsPerDagVdWeekActivity, getString(R.string.SwipeLinksOfRechts))
    }
    ...
    private fun toondataBackground() {
        ...
        AsyncGetStatistiekenPerDagVdWeekTask(this).execute(context)
    }

    private fun toonStatistiekenPerDagVdWeek(stats: ArrayList<StatistiekDagVdWeek>) {
        val tvDagVdWeekTitel = findViewById<TextView>(R.id.tvDagVdWeekTitel)

        if (mAllesJaarMaand === Helper.Periode.Alles) {
            tvDagVdWeekTitel.text = String.format("%s", getString(R.string.AantalPerDagVdWeek))
        }

        if (mAllesJaarMaand === Helper.Periode.Jaar) {
            tvDagVdWeekTitel.text = String.format("%s %s", getString(R.string.AantalPerDagVdWeek), String.format(getString(R.string.Jaartal), Integer.toString(mJaar)))
        }

        if (mAllesJaarMaand === Helper.Periode.Maand) {
            val dat = DateTime(mJaar, mMaand, 1, 0, 0)
            val mnd = dat.toString("MMM", Locale.getDefault()).replace(".", "")
            tvDagVdWeekTitel.text = String.format("%s %s", getString(R.string.AantalPerDagVdWeek), String.format(getString(R.string.JaartalEnMaand), mnd, Integer.toString(mJaar)))
        }

        val chart = findViewById<BarChart>(R.id.bcPerDagVdWeek)
        ...
        val labels = ArrayList(Arrays.asList("Ma", "Di", "Wo", "Do", "Vr", "Za", "Zo"))
        ...
        for (i in stats.indices) {
            ...
            dataT[dag].setVals(floatArrayOf(aantalNat.toFloat(), aantalDroog.toFloat()))
        }
    ...
    private class AsyncGetStatistiekenPerDagVdWeekTask internal constructor(context: StatsPerDagVdWeekActivity) : AsyncTask<Context, Void, ArrayList<StatistiekDagVdWeek>>() {

        private val activityWeakReference: WeakReference<StatsPerDagVdWeekActivity> = WeakReference(context)
        ...
        override fun onPostExecute(stats: ArrayList<StatistiekDagVdWeek>) {
            ...
            activity?.toonStatistiekenPerDagVdWeek(stats)
        }
    }
